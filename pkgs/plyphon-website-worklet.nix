# The AudioWorklet web demo: the examples built for the worklet backend, which runs audio on a
# dedicated Web Audio thread via WASM threads (SharedArrayBuffer). It needs a *nightly* toolchain
# (`-Z build-std` to recompile `std` with atomics) and the shared-memory build flags from
# `wasm-threads-env.nix`; hence a separate derivation from the default `plyphon-website` (which
# stays on stable + the legacy Web Audio backend as the no-headers-required fallback).
#
# Currently a single pilot page (`sine`); more examples follow during rollout.
#
