...
## UGens (261 of scsynth's ~382 standard DSP UGens, grouped by category)
...
- [ ] **Buffer playback / recording** - have PlayBuf, DiskIn, RecordBuf (record into a buffer with overdub/run/loop/doneAction), BufWr (write channels at a phase index), DiskOut (stream channels out to a cued recording buffer, drained off the audio thread to a sink), BufRd (read a buffer wherever a `phase` input points - the read counterpart to BufWr, one output per channel, `interpolation` none/linear/cubic and `loop` wrap-vs-clamp, sharing `sc_loop` and the interp helpers); missing VDiskIn (variable-rate disk streaming - needs resampling the disk-stream ring at a controllable rate, deferred at the stream/host boundary)
