...
impl World {
    ...
    fn build_graph(&mut self, def: &Arc<GraphDef>, aux: AuxSlots) -> Result<Graph, AuxSlots> {
        ...
        local_bytes.fill(0);
        // Every graph-local buffer (`LocalBuf`) starts without storage; each one's storage is
        // appended to the block when it starts, on the first block.
        lbuf_meta_bytes.fill(0);
