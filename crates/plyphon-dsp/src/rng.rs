...
impl Rng {
    ...
    /// Reinitialize the stream with scsynth's `RGen::init` integer hash and state constants.
    pub fn reseed(&mut self, seed: u32) {
        ...
        self.s1 = 1_243_598_713 ^ hash;
        if self.s1 < 2 {
            self.s1 = 1_243_598_713;
        }
        self.s2 = 3_093_459_404 ^ hash;
        if self.s2 < 8 {
            self.s2 = 3_093_459_404;
        }
        self.s3 = 1_821_928_721 ^ hash;
        if self.s3 < 16 {
    ...
    /// A double-precision unipolar sample with the same bit construction as scsynth's `RGen::drand`.
    #[inline]
    fn next_unipolar_f64(&mut self) -> f64 {
        f64::from_bits(0x4130_0000_0000_0000 | self.next_u32() as u64) - 1_048_576.0
    }

    /// Draw from scsynth's double-precision exponential range between `lo` and `hi`.
    #[inline]
    pub fn next_exprand_f64(&mut self, lo: f64, hi: f64) -> f64 {
        lo * crate::math::exp(crate::math::ln(hi / lo) * self.next_unipolar_f64())
    }

    /// An integer from scsynth's `RGen::irand`, `floor(scale * drand)`.
    #[inline]
